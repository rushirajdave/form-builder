...
/*function generate_ul(ar)
{
    ...
    {
        var res = '<ul class="sorting-field">';
        var cnt1 = 0;
        var cnt2 = 0;
        var arr1 = new Array();
        var arr2 = new Array();
        for(var kt=0;kt<ar.length;kt++)
        {
            if(kt%2==0)
            {
                arr1[cnt1] = ar[kt];
        ...
        for(var vs =0;vs<arr1.length;vs++)
        {
            res += '<li value="'+arr2[vs]+'">'+arr1[vs]+'</li>';
        }
        return res;
    }
}*/
function orange_star(a,l)
{
    //alert('orange_star'+a+l);
    //alert($('#start_img_'+l+0).attr('src'));
    for(var i=0;i<=a;i++)
    {
        $('#start_img_'+l+i).attr('src','orange_star.png');
    }
    
}
function gray_star(b,l)
{
    //alert('gray_star');
    for(var i=b;i>=0;i--)
    {
        $('#start_img_'+l+i).attr('src','gray_star.png');
    }
    
}
function fix_star(c,l)
{
    $('.rate-'+l).attr('src','gray_star.png');
    for(var i=0;i<=c;i++)
    {
        $('#start_img_'+l+i).attr('src','orange_star.png');
    }    
    $('.rate-'+l).attr('onmouseover','');
    $('.rate-'+l).attr('onmouseout','');
    $('#res-'+l).text(eval(c+1)+'/'+l);
}
function disablen(x,l)
{
    if($(x).prop('checked') == true)
       {
       $('#res-'+l).text(''); 
        $(x).siblings('a').attr('style','display:none;'); 
        $('#res-'+l).text('Not Applicable for rating ');
        }
    else
    {
        $(x).siblings('a').attr('style','display:;'); 
        $('#res-'+l).text('');
    }
}
function add_rate(len)
{
    if(len == 5)
    {
    var str ='<div class="rate-holder-5">';
    for (var i = 0; i< len; i++) {
        str += '<a class="rate_anchor-5" ><img class="rate-5" id="start_img_5'+i+'" src="gray_star.png" onmouseover="orange_star('+i+','+len+')" onmouseout="gray_star('+i+','+len+')" onclick="fix_star('+i+','+len+')"></a>';       
        }
        str +=' &nbsp;<span id="res-5"></span><br/><input type="checkbox" name="na5" onchange="disablen(this,'+len+')">Not Applicable</div>';
    }
    else if(len == 10)
    {
        var str ='<div class="rate-holder-10">';
    for (var i = 0; i< len; i++) {
        str += '<a class="rate_anchor-10" ><img class="rate-10" id="start_img_10'+i+'" src="gray_star.png" onmouseover="orange_star('+i+','+len+')" onmouseout="gray_star('+i+','+len+')" onclick="fix_star('+i+','+len+')"></a>';       
        }
        str +='&nbsp;<span id="res-10"></span><br/><input type="checkbox" name="na10" onchange="disablen(this,'+len+')">Not Applicable</div>';
    }
...
function formBuilderHelpersFn(e, t) {
    ...
    return t.events = formBuilderEventsFn(), a.attrString = function(e) {
        ...
        if (-1 === t.indexOf("ui-sortable-handle")) {
            ...
            r && (n.style = r), l.match(/(select|checkbox-group|radio-group|sortable-field)/) && (n.values = [], n.multiple = $('[name="multiple"]', e).is(":checked"), $(".sortable-options li", e).each(function() {
                var e = {};
        ...
        switch (t.type) {
            ...
            case "select":
                ...
                for (t.values.reverse(), t.placeholder && (i += "<option disabled selected>" + t.placeholder + "</option>"), s = t.values.length - 1; s >= 0; s--) {
                    var d = t.values[s].selected && !t.placeholder ? "selected" : "";
                    i += '<option value="' + t.values[s].value + '" ' + d + ">" + t.values[s].label + "</option>"
                }
                l = "<" + t.type + ' class="' + t.className + '" ' + c + ">" + i + "</" + t.type + ">";
                break;
            case "checkbox-group":
            case "radio-group":
                ...
                for (t.values.reverse(), s = t.values.length - 1; s >= 0; s--) {
                    var m = t.values[s].selected ? "checked" : "",
                        f = u + "-" + o + "-" + s;
                    l += '<div><input type="' + u + '" class="' + t.className + '" name="' + p + '" id="' + f + '" value="' + t.values[s].value + '" ' + m + '/><label for="' + f + '">' + t.values[s].label + "</label></div>"
                }
                if (t.enableOther) {
                    var v = p + "-other",
                        h = {
                    ...
                    }, l += "<div>" + g.outerHTML + '<label for="' + v + '">' + e.messages.other + '</label> <input type="text" id="' + v + '-value" style="display:none;" /></div>'
                }
                break;
            case "text":
            case "password":
            ...
            case "sortable-field":
                var u = t.type.replace("-group", ""),
                    p = u + "-" + o;
                    l = '<ul class="sortable-field-group">'
                for (t.values.reverse(), s = t.values.length - 1; s >= 0; s--) {
                    var m = t.values[s].selected ? "checked" : "",
                        f = u + "-" + o + "-" + s;
                    l += '<li type="' + u + '" class="' + t.className + '" name="' + p + '" id="' + f + '" value="' + t.values[s].value + '" ' + m + '><label for="' + f + '">' + t.values[s].label + "</label></li>"
                }
                l += '</ul>'
                if (t.enableOther) {
                    var v = p + "-other",
        ...
        var l = ["checkbox", "checkbox-group", "radio-group","sortable-field"],
        ...
        if (p) {
            ...
            d.push(p)
        } else a.inArray(r, l) || d.push("form-control");
        ...
        var t = document.getElementById(e),
            ...
            s = $(".frm-holder", t);
            //********************************************************************************
            //rushiraj - following code works but as we dont want dynamic number of stars it is commented 
            //uncomment in case you want dynamic stars to be generated


            //alert($(".prev-holder").attr("style"));
            /*if($(".prev-holder").attr("style") == "display: none;")
            {
               if($('#first-time').attr('maxlength') == 1)
               {
                 $('#first-time').remove();    
                $('.rating-field').append(add_rate($('.rating-field').attr('data-star-cnt')));
               }             
        }*/ 
        //***********************************************************************************************
        /*if($('#'+e).attr('type') == "sortablefield")
        {
            var cnt = 0;
            var vm = new Array();
            var x = $('.sortable-options').children('li');
            //alert(x.length);
            for(var aj=0;aj<x.length;aj++)
            {
                var q = $(x[aj]).children('input');
                //alert(q.length);
                for(var rd=0;rd<q.length;rd++)
                {   
                    if ($(q[rd]).attr("type") == 'text') 
                    {
                        vm[cnt] = $(q[rd]).val();
                        cnt++;
                    }
           ...
           if($(".prev-holder").attr("style") == "display: none;")
            {
                ...
                $('.prev-holder').html(generate_ul(vm));
    ...
    var t = function(t, a) {
        var s = this,
            l = {
                ...
                controlOrder: ["autocomplete", "button", "checkbox", "checkbox-group", "date", "file", "header", "hidden", "paragraph", "number", "radio-group", "select", "text", "textarea","rating5star","rating10star","sortable-field"],
                dataType: "xml",
                ...
                messages: {
                    ...
                    radio: "Radio",
                    rating5star: "5 Star Rating",
                    rating10star: "10 Star Rating",
                    ...
                    sortablefield: 'Sortable Options',
                    style: "Style",
                    styles: {
                        btn: {
                            "default": "Default",
        ...
        var d = r + "-fld-1",
            ...
            p = [{
                ...
                label: o.messages.rating5star,
                attrs: {
                    type: "rating5star",
                    className: "rating5star",
                    name: "rating5star"
                }
            },
                {
                label: o.messages.rating10star,
                attrs: {
                    type: "rating10star",
                    className: "rating10star",
                    name: "rating10star"
                }
            },
            {
                label: o.messages.sortablefield,
                attrs: {
                    type: "sortable-field",
        ...
        for (var f = e(m), v = p.length - 1; v >= 0; v--) {
            ...
            h.data("newFieldData", p[v]);
            var g = c.markup("span", p[v].label);
            h.html(g).appendTo(f)
        }
        ...
        i.on("change blur keyup", ".form-elements input, .form-elements select, .form-elements textarea", O), e("li", f).click(function(t) {
        ...
        var $ = function(e) {
            ...
            L = function U(e) {
                var t, U = [],
                    a = "",
                    s = ["select", "checkbox-group", "radio-group","sortable-field"],
                    l = function() {
            ...
            B = function _(e, t) {
                var a = t[e] || "",
                    ...
                    _ = '<input type="number" value="' + a + '" name="' + e + '" placeholder="' + l + '" class="fld-' + e + ' form-control" id="' + e + "-" + d + '">';
            ...
            j = function(e, t) {
                var a = ["text", "textarea", "select"],
                    s = ["header"],
                    l = ["paragraph"],
                    n = ["checkbox", "select", "checkbox-group", "date", "autocomplete", "radio-group","sortable-field" ,"hidden", "button", "header", "number"],
                    r = t[e] || "",
                    i = o.messages[e];
                ...
                if ("placeholder" !== e || c.inArray(t.type, a) || f.push(!0), "name" === e && c.inArray(t.type, s) && f.push(!0), "maxlength" === e && c.inArray(t.type, n) && f.push(!0), !f.some(function(e) {
                    ...
                    m += "label" === e && c.inArray(t.type, l) ? '<textarea name="' + e + '" placeholder="' + p + '" class="fld-' + e + ' form-control" id="' + e + "-" + d + '">' + r + "</textarea>" : '<input type="text" value="' + r + '" name="' + e + '" placeholder="' + p + '" class="fld-' + e + ' form-control" id="' + e + "-" + d + '">', m = '<div class="form-group ' + e + '-wrap">' + v + " " + m + "</div>"
                }
                return m
            },
            H = function(e) {
            ...
            P = function(t) {
                ...
                var u = t.required ? 'style="display:inline"' : "";
                r += '<span class="required-asterisk" ' + u + "> *</span>", r += c.markup("div", "", {
            ...
            R = function(e, t, a) {
                ...
                n.push(c.markup("a", o.messages.remove, u));
                var p = c.markup("li", n);
                return p.outerHTML;
            };
        i.on("click touchstart", ".remove", function(t) {
            ...
            /*if(e(this).parents('.rating-field').attr('type') == 'rating')
            {
                e(this).parents('.rating-field').attr('data-star-cnt',c.forceNumber(e(this).val()));
            }*/
            ...
            if (document.addEventListener("modalClosed", function() {
                ...
                c.confirm([u, p], d, l), r.addClass("deleting")
            } else d(r)
        ...
        }), n.parent().find('p[id*="ideaTemplate"]').remove(), n.wrap('<div class="template-textarea-wrap"/>'), M(), i.css("min-height", f.height()), o.stickyControls && c.stickyControls(i, m), document.dispatchEvent(s.events.loaded), s
    };
    e.fn.formBuilder = function(a) {
        return this.each(function() {
            ...
            if (e(l).data("formBuilder")) {
                ...
                o.before(l), o.remove(), s = new t(a, l), e(l).data("formBuilder", s)
            } else s = new t(a, l), e(l).data("formBuilder", s)
        })
    }
}(jQuery),
function(e) {
            ...
            l.childNodes.length >= 1 && (a += "<form-template>\n	<fields>", t.forEach(l.childNodes, function(l, o) {
                ...
                if (!n.hasClass("disabled")) {
                    ...
                    ! function() {
                        ...
                        }), r.length && (l.role = r.join(",")), i && (l.other = "true"), l = t.trimObj(l), l = t.escapeAttrs(l), c = l.type.match(/(select|checkbox-group|radio-group|sortable-field)/), d = "", c && (d = s(n)), u = t.markup("field", d, l), a += "\n		" + u.outerHTML
                    }()
                }
            }), a += "\n	</fields>\n</form-template>")
        }), a
